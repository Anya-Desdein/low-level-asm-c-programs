...
RGB hsv_to_rgb(HSV hsv) {
	...
	rgb.r = (int)(r * 255 + 0.5f);
	rgb.g = (int)(g * 255 + 0.5f);
	rgb.b = (int)(b * 255 + 0.5f);
...
size_t count_digits(size_t num) {
 ...
 SlicedArr slice(
	...
	*/
	SlicedArr sliced;
	sliced.element_len     = max_text_size + 1; // Include space for null terminator	
	...
	for (uint64_t i=0; i < full_chunks; i++) {
		...
		size_t  curr_el      = (size_t)i * sliced.element_len,
			curr_el_size = sliced.element_len - 1,
			last_char    = curr_el + sliced.element_len - 1; 

		memcpy( sliced.array_pt + 
			curr_el,
		...
		printf("%s\n", (char *)(sliced.array_pt + curr_el));
	}
	
	if(remainder) {
	
		size_t  last_el         = (size_t)(sliced.array_len - 1) * sliced.element_len;
			
		char   *last_el_wbase   = last_el + sliced.array_pt,
		       *last_char_wbase = last_el_wbase + sliced.element_len - 1; 

		memcpy( last_el_wbase, 
			str + last_el,
			remainder
			);
	
		memcpy( last_char_wbase,
			&null_term,
			1);
	}
...
void generate_result_table() {
	...
	uint64_t max_subh = 0, max_subh_rounded = 0, subh_count = 0,
	         subh_size = ARRAY_SIZE(subh), subh_len[subh_size];
	for (uint64_t i=0; i < subh_size; i++) {

		assert((subh_size && subh_size != 0) 
			&& "Invalid or missing subh_size in generate_result_table()");	
		
		assert( subh[i] 
			&& "Missing subh element in generate_result_table()");

		subh_len[i] = strlen(subh[i]);

		if (subh_len[i] > max_subh)
			max_subh = subh_len[i];
	} 
      
      	max_subh_rounded = align_to(max_subh, column_len_raw);
	...
	for (size_t i=0; i < subh_count; i++) {	
		...
		assert(subh_rows[i].element_len && subh_rows[i].element_len != 0 
			&& "Invalid subh_rows[i].element_len in generate_result_table()");
		assert(subh_rows[i].array_pt    && subh_rows[i].array_pt    != 0 
	...
	for (size_t i=0; i < max_rows; i++) {	
		for (size_t j=0; j < subh_count; j++) {
			if (i < subh_rows[j].array_len) {
				...
				print_column_el(table_len, empty, subh_align, &hsv);
			}
		}
	}
	puts("");

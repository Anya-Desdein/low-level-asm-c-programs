...
static ssize_t
sock_send(const int fd, const char *buf, const ssize_t bufsize) {
...
void priv_id(
	...
	do {
		const char symbol = msg[input_iter];
		const int  isdig  = symbol;
		...
		id_buf[input_iter-1] = symbol;

		input_iter++;
	} while (input_iter < (max_id_len+1) && msg[input_iter] != ' ' && msg[input_iter] != '\0');
...
void priv_name(
	...
	int match=0, input_iter=1;
	const int max_name_len = CLIENT_MAX_ALIAS-1;

	char name_buf[max_name_len+1];;
	memset(name_buf, 0, max_name_len+1);

	do {
		const char symbol = msg[input_iter];
		const int  isdig  = isdigit(symbol);
		const int  isal   = isalpha(symbol);
		...
		if (
			...
			printf("error: %c\n", symbol);
			static const char msg_nanid[] = "//n: incorrect first argument: illegal character in name\n";
			printf("%s\n", msg_nanid);
			send_err = sock_send(
				users->clients[user_id],
				msg_nanid,
			...
			if (max_name_len < input_iter) {
				printf("error: %c\n", symbol);
				static const char msg_idtoobig[] = "//n: incorrect first argument: name is larger than maximum possible name lenght\n";
				printf("%s\n", msg_idtoobig);
				send_err = sock_send(
					users->clients[user_id],
					msg_idtoobig,
		...
		name_buf[input_iter-1] = symbol;

		input_iter++;
	} while (input_iter < (max_name_len+1) && msg[input_iter] != ' ' && msg[input_iter] != '\0');

	char client_name[CLIENT_MAX_ALIAS];

	for (int i=0; i < CLIENT_MAX; i++) {
		strcpy(client_name, users->aliases[i]);
		if(!client_name[0])
			continue;

		match = strcmp(client_name, name_buf);
		if(match == 1)
			break;
	}
...
void remove_name(
	...
	ssize_t send_err = sock_send(users->clients[user_id], msg_succ, ARRAY_SIZE(msg_succ));
	if (send_err == -1)
		printf("/remove_name: sock_send: failed to send user message\n");
...
void help(const char *msg, const ssize_t msg_size, Users *users, int user_id) {
	...
	send_err = sock_send(users->clients[user_id], msg_begin, ARRAY_SIZE(msg_begin));
	if (send_err == -1) {
	...
	send_err = sock_send(users->clients[user_id], separatorl, separatorl_size);
	if (send_err == -1) {
		printf("/help: sock_send: failed to send user message\n");
		return;
	}
	...
	send_err = sock_send(users->clients[user_id], msg_list_header, ARRAY_SIZE(msg_list_header));
	if (send_err == -1) {
		printf("/help: sock_send: failed to send user message\n");
		return;
	}

	send_err = sock_send(users->clients[user_id], separator, separator_size);
	if (send_err == -1) {
		printf("/help: sock_send: failed to send user message\n");
		return;
	}
	...
	for (size_t i=0; i < command_count; i++) {
		...
		send_err = sock_send(users->clients[user_id], n, n_len);
		if (send_err == -1) {
			printf("/help: sock_send: failed to send user message\n");
			return;
		}

		send_err = sock_send(users->clients[user_id], subh, subh_len);
		if (send_err == -1) {
			printf("/help: sock_send: failed to send user message\n");
			return;
		}

		send_err = sock_send(users->clients[user_id], cmd_usage, cmd_usage_len);
		if (send_err == -1) {
			printf("/help: sock_send: failed to send user message\n");
			return;
		}
		...
		send_err = sock_send(users->clients[user_id], separators, separators_size);
		if (send_err == -1) {
			printf("/help: sock_send: failed to send user message\n");
			return;
		}
